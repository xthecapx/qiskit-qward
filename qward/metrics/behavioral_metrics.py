...
class BehavioralMetrics(MetricCalculator):
    ...
    def _calculate_critical_depth(self) -> float:
        ...
        two_qubit_nodes = [
            node for node in self._dag_circuit.topological_op_nodes()
        ...
        critical_op_nodes = [
            node for node in longest_path if isinstance(node, DAGOpNode)
        ]
    ...
    def _calculate_liveness(self) -> float:
        ...
        L = active_counts / (n_qubits * depth)
        return L
