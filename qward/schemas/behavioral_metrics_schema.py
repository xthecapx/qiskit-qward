...
class BehavioralMetricsSchema(BaseModel):
    ...
    normalized_depth: float = Field(..., ge=0.0, description="Depth of circuit after transpilation to basis gates ['rx', 'ry', 'rz', 'cx']")
    
    # Program communication requirements
    program_communication: float = Field(..., ge=0.0, le=1.0, description="Normalized average degree of interaction graph (communication requirements)")
    
    # Critical path analysis
    critical_depth: float = Field(..., ge=0.0, le=1.0, description="Ratio of two-qubit interactions on critical path to total two-qubit interactions")
    
    # Measurement operations
    measurement: float = Field(..., ge=0.0, le=1.0, description="Ratio of layers with mid-circuit measurement/reset operations to total circuit depth")
    
    # Qubit activity patterns
    liveness: float = Field(..., ge=0.0, le=1.0, description="Ratio of active qubit-time steps to total qubit-time steps")
    
    # Parallelism and cross-talk susceptibility
    parallelism: float = Field(..., ge=0.0, le=1.0, description="Parallelism factor indicating susceptibility to cross-talk")
