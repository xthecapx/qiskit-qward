...
class ElementMetricsSchema(BaseModel):
    ...
    percent_q_in_cnot: float = Field(..., ge=0.0, le=1.0, description="Ratio of qubits affected by CNOT gates")
    avg_cnot: float = Field(..., ge=0.0, description="Average number of CNOT gates directed to any qubit in a circuit")
    max_cnot: int = Field(..., ge=0, description="Maximum number of CNOT gates directed to any qubit of a circuit")
    
    # Toffoli gate metrics
    no_toff: int = Field(..., ge=0, description="Number of Toffoli gates")
    percent_q_in_toff: float = Field(..., ge=0.0, le=1.0, description="Ratio of qubits affected by Toffoli gates")
    avg_toff: float = Field(..., ge=0.0, description="Average number of Toffoli gates targeting any qubit of a circuit")
    max_toff: int = Field(..., ge=0, description="Maximum number of Toffoli gates targeting any qubit of a circuit")
    
    # General gate metrics
    no_gates: int = Field(..., ge=0, description="Total number of gates in the circuit")
    no_c_gates: int = Field(..., ge=0, description="Total number of controlled gates in the circuit")
    percent_single_gates: float = Field(..., ge=0.0, le=1.0, description="Ratio of single gates to total gates")
    ...
    percent_anc: float = Field(..., ge=0.0, le=1.0, description="Ratio of ancilla (auxiliary) qubits in the circuit")
    ...
    def validate_t_no_p(cls, v, info):
        ...
        if hasattr(info, 'data'):
            expected = info.data.get('no_p_x', 0) + info.data.get('no_p_y', 0) + info.data.get('no_p_z', 0)
            if v != expected:
                raise ValueError(f"Total Pauli gates ({v}) must equal no_p_x + no_p_y + no_p_z ({expected})")
        return v
        ...
        if hasattr(info, 'data'):
            ...
            if total_gates > 0:
                ...
                if abs(v - expected) > 0.001:  # Allow small floating point errors
                    raise ValueError(f"Single gates percentage ({v}) must equal t_no_sqg / no_gates ({expected})")
        return v
