...
# qward imports for metrics and visualization
...
class QuantumCircuitExecutor:
    ...
    def _create_depolarizing_noise_model(self, noise_level: float) -> NoiseModel:
        ...
        noise_model.add_all_qubit_quantum_error(depol_error_1q, ["u1", "u2", "u3", "h", "x", "y", "z", "s", "t"])
    ...
    def _create_pauli_noise_model(self, noise_level: float) -> NoiseModel:
        ...
        pauli_error_1q = pauli_error([("X", pauli_prob), ("Y", pauli_prob), ("Z", pauli_prob), ("I", identity_prob)])
        noise_model.add_all_qubit_quantum_error(pauli_error_1q, ["u1", "u2", "u3", "h", "x", "y", "z", "s", "t"])
        
        # Add Pauli error to all two qubit gates
        pauli_prob_2q = noise_level / 3
        identity_prob_2q = 1 - noise_level
        pauli_error_2q = pauli_error([("XX", pauli_prob_2q), ("YY", pauli_prob_2q), ("ZZ", pauli_prob_2q), ("II", identity_prob_2q)])
        ...
        readout_error = ReadoutError([[1 - readout_prob, readout_prob], 
    ...
    def _get_noise_model(self, noise_model_config: Union[str, NoiseModel, None], noise_level: float = 0.05) -> Optional[NoiseModel]:
    ...
    def simulate(self, circuit: QuantumCircuit, success_criteria=None, show_results: bool = False,
                 noise_model: Union[str, NoiseModel, None] = None, noise_level: float = 0.05) -> Dict[str, Any]:
    ...
    def get_circuit_metrics(self, circuit: QuantumCircuit, job=None, success_criteria=None) -> Dict[str, pd.DataFrame]:
    ...
    def _display_results(self, scanner: Scanner, data: Dict[str, Any], 
                        noise_model: Union[str, NoiseModel, None] = None, noise_level: float = 0.05) -> None:
    ...
    def run_qbraid(self, circuit: QuantumCircuit, device_id: Optional[str] = None, success_criteria=None) -> Dict[str, Any]:
        ...
        try:
            ...
            device_id = device_id or QbraidDevice.RIGETTI.value
            ...
            # Submit job
            job = qbraid_device.run(transpiled_circuit, shots=self.shots)
            ...
            while time.time() - start_time < self.timeout:
                ...
                if status_str in ['COMPLETED', 'DONE']:
                    ...
                    metrics_data = self.get_circuit_metrics(circuit, success_criteria=success_criteria)
            ...
            metrics_data = self.get_circuit_metrics(circuit, job=job, success_criteria=success_criteria)
