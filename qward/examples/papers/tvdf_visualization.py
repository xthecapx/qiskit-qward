"""
TVDF (Total Variation Distance Fidelity) — 3Blue1Brown-Style Visualization
===========================================================================
ManimCE animation showing TVD Fidelity degradation under noise.

Formula:
    TVD  = (1/2) Σ |p_ideal(x) - p_measured(x)|
    TVDF = 1 - TVD

Render:
    uv run manim -pql qward/examples/papers/tvdf_visualization.py TVDFDegradation
High quality:
    uv run manim -pqh qward/examples/papers/tvdf_visualization.py TVDFDegradation
"""
...
# ══════════════════════════════════════════════════════════════════
# TVDF Degradation — animated noise sweep
# ══════════════════════════════════════════════════════════════════
class TVDFDegradation(Scene):
    def construct(self):
        self.camera.background_color = BG_COLOR

        section_title = Text(
            "TVD Fidelity Degradation Under Noise",
            font_size=34, color=TVDF_COLOR,
        ).to_edge(UP, buff=0.4)
        self.play(Write(section_title))

        # ── Formula card ──────────────────────────────────────────
        formula = MathTex(
            r"\text{TVDF} = 1 - \frac{1}{2}\sum_x"
            r"\left|p_{\text{ideal}}(x) - p_{\text{meas}}(x)\right|",
            font_size=32, color=TVDF_COLOR,
        ...
        def tvdf_func(t):
            # TVD = 0.5 * (|0-0.25t| + |1-(1-0.75t)| + |0-0.25t| + |0-0.25t|)
            #     = 0.5 * (0.25t + 0.75t + 0.25t + 0.25t)
            #     = 0.5 * 1.5t = 0.75t
            # TVDF = 1 - 0.75t
        ...
        # Plot TVDF(t) curve — linear: 1 - 0.75t
        tvdf_curve = curve_axes.plot(
            lambda t: tvdf_func(t), x_range=[0, 1, 0.01],
            color=TVDF_COLOR, stroke_width=3,
        )
        ...
        # TVDF value label
        tvdf_value_label = always_redraw(lambda: MathTex(
            rf"\text{{TVDF}} = {tvdf_func(tracker.get_value()):.2f}",
            font_size=28, color=TVDF_COLOR,
        ...
        self.play(Create(tvdf_curve), run_time=1.5)
        self.add(moving_dot, vert_dash, tvdf_value_label)
        self.wait(0.5)
