...
def parse_grover_config(config_id: str) -> tuple[int, int]:
    ...
    if (
        config_id.startswith("S")
        and len(config_id) > 1
        and config_id[1].isdigit()
        and "-" in config_id
    ):
...
def classify_region(
    success_rate: float, random_chance: float, config: RegionConfig
) -> tuple[str, float]:
    """
...
def print_summary_table(
    ...
    sorted_configs = sorted(config_groups.keys(), key=lambda c: (config_groups[c][0].num_qubits, c))
    ...
    for config_id in sorted_configs:
        ...
        ref_result = next(
            (r for r in config_results if r.noise_model == reference_noise_model), None
        )
...
def main():
    ...
    parser.add_argument("input_file", help="Path to campaign summary JSON file")
    parser.add_argument(
        "--algorithm",
        "-a",
