...
def calculate_statistical_analysis(success_rates: List[float], config_id: str, noise_model: str) -> Dict[str, Any]:
    """
...
class IBMExperimentBase(ABC, Generic[ConfigT]):
        ...
        if success_rates:
            ...
            if len(success_rates) >= 2:
                analysis = calculate_statistical_analysis(
                    success_rates, 
                    config_desc.get('config_id', 'unknown'),
                    "IBM-QPU"
                )
    ...
    def _print_analysis(self, result: Dict[str, Any], config: ConfigT) -> None:
        ...
        for run in result.get("individual_results", []):
            ...
            print(f"  Original depth: {run.get('circuit_depth', '?')}, Transpiled: {run.get('transpiled_depth', '?')}")
    ...
    def create_argument_parser(self) -> argparse.ArgumentParser:
        ...
        parser.add_argument(
            "--channel",
            default=None,
            help="IBM Quantum channel ('ibm_quantum' or 'ibm_cloud')"
        )
        parser.add_argument(
            "--token",
            default=None,
        ...
        parser.add_argument(
            "--instance",
            default=None,
            help="IBM Quantum instance (e.g., 'ibm-q/open/main')"
        )
        return parser
    ...
    def list_configs(self) -> None:
        ...
        if priority_configs:
            ...
            for cfg in priority_configs:
                ...
                print(f"{cfg['config_id']:<12} {cfg['qubits']:<8} {cfg['depth']:<8} "
