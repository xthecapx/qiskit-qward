...
def job_success_statistical(
    ...
    return {
        ...
        "interpretation": (
            ...
            if is_success else
            f"Success rate not significantly above random (p={result.pvalue:.4f})"
        )
    }
...
def job_success_quantum_advantage(
    ...
    return {
        ...
        "interpretation": (
            ...
            if is_success else
            f"No quantum advantage: {advantage_ratio:.2f}x classical (need {advantage_factor}x)"
        )
    }
...
def job_success_theoretical(
    ...
    return {
        ...
        "interpretation": (
            ...
            if is_success else
            f"Outside tolerance: {rate:.3f} vs {theoretical_prob:.3f} (±{deviation:.3f})"
        )
    }
...
def batch_success_mean(
    ...
    return {
        ...
        "interpretation": (
            ...
            if is_success else
            f"Batch failed: mean={mean_rate:.3f} < {threshold}"
        )
    }
...
def batch_success_min(
    ...
    return {
        ...
        "interpretation": (
            ...
            if is_success else
            f"Some jobs failed: min={min_rate:.3f} < {threshold}"
        )
    }
...
def batch_success_median(
    ...
    return {
        ...
        "interpretation": (
            ...
            if is_success else
            f"Batch failed (median): {median_rate:.3f} < {threshold}"
        )
    }
...
def batch_success_consistency(
    ...
    return {
        ...
        "interpretation": (
            ...
            if is_success else
            f"Inconsistent results: std={std_rate:.3f} > {max_std}"
        )
    }
...
def print_evaluation_summary(evaluation: Dict[str, Any]) -> None:
    ...
    print(f"  Statistical success: {evaluation['jobs_statistical_success']}/{evaluation['num_jobs']}")
