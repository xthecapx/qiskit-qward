...
This folder contains graphs generated from QPU execution results for Grover's algorithm. By default, the visualizer auto-loads provider folders under `data/`, using `data/qpu/raw` (IBM) and `data/qpu/aws` (AWS). Use these plots to see **when results stop being useful** as circuit size or depth increases.
...
```bash
python grover_qpu_visuals.py --provider aws
