...
def parse_raw_file(file_path: Path) -> Optional[RawResult]:
    ...
    try:
        ...
        if algorithm == "GROVER":
            ...
            result.num_marked = first_result.get("num_marked", len(result.marked_states) if result.marked_states else 1)
        elif algorithm == "QFT":
...
def print_analysis_table(results: list[RawResult], reference_noise: str) -> str:
    ...
    sorted_configs = sorted(groups.keys(), 
                           key=lambda c: (groups[c][0].num_qubits, c))
    ...
    for config_id in sorted_configs:
        ...
        ref_result = next((r for r in config_results if r.noise_model == reference_noise), config_results[0])
...
def generate_grover_qpu_configs(results: list[RawResult], reference_noise: str, max_configs: int = 15) -> str:
    ...
    lines.append("")
    lines.append("from qward.examples.papers.grover.grover_configs import get_config, get_noise_config")
    lines.append("")
    ...
    for i, r in enumerate(sorted_results[:max_configs], 1):
        ...
        lines.append(f"    # Priority {i}: {r.config_id} - {format_percentage(r.mean_success_rate)} expected ({r.region})")
        lines.append(f"    {{")
    ...
    total_runs = sum(5 if "Region 1" in r.region else (3 if "Region 2" in r.region else 2) 
...
def generate_qft_qpu_configs(results: list[RawResult], reference_noise: str, max_configs: int = 15) -> str:
...
def main():
    parser = argparse.ArgumentParser(description="Generate QPU configurations from raw simulator data")
    parser.add_argument("--algorithm", "-a", required=True, choices=["grover", "qft"],
                       help="Algorithm type")
    parser.add_argument("--data-dir", "-d", required=True,
                       help="Path to raw data directory")
    parser.add_argument("--noise-models", "-n", nargs="+",
                       default=["IBM-HERON-R3", "IBM-HERON-R2", "IBM-HERON-R1", "RIGETTI-ANKAA3"],
                       help="Noise models to include")
    parser.add_argument("--reference-noise", "-r", default="IBM-HERON-R2",
                       help="Reference noise model for classification")
    parser.add_argument("--max-configs", "-m", type=int, default=15,
                       help="Maximum configs to output")
    ...
    if args.algorithm == "grover":
        output_lines.append(generate_grover_qpu_configs(results, args.reference_noise, args.max_configs))
    else:
        output_lines.append(generate_qft_qpu_configs(results, args.reference_noise, args.max_configs))
