...
POLICIES = (
    "inspect_all", "unique_mode", "contrast_0.2", "contrast_0.5", "contrast_0.8",
    "rank_0.05", "rank_0.01", "contrast_0.2_and_rank_0.05", "gap_0.05",
)
...
def screen(counts: np.ndarray) -> dict:
    ...
    return {
        "n1": n1, "n2": n2, "contrast": contrast,
        "pair_mass": (n1 + n2) / total, "top_mass": n1 / total,
        "gap": (n1 - n2) / total, "rank_p": rank_p(n1, n2),
        "unique": n1 > n2,
    }
...
def target_metrics(hist: dict, expected: set) -> dict:
    ...
    return {
        "target_mass": mass / total, "wrong_max_count": competitor,
        "target_mean_signed": signed, "target_dsr": max(0.0, signed),
        ...
        "target_min_count": min(target_counts), "target_max_count": max(target_counts),
    }
...
def load_data() -> tuple[pd.DataFrame, list]:
    ...
    for row_id, row in raw.iterrows():
        ...
        cohort = "primary" if row.algorithm != "TELEPORTATION" else (
            "teleportation_simulator" if row.execution_type == "SIMULATION"
            else "teleportation_hardware"
        )
        record = {key: row[key] for key in (
            "algorithm", "config_id", "backend_name", "execution_type", "shots",
            "num_qubits", "optimization_level", "transpiled_depth", "result_id", "source_file",
        )}
        record.update(
            row_id=row_id, cohort=cohort, output_width=width, k=len(expected),
            target_group="singleton" if len(expected) == 1 else "multiple",
            register_aligned=aligned, candidate_correct=candidate_correct,
            candidate=str(tied[0]) if len(tied) == 1 else "TIED",
            stored_dsr=float(row.dsr_michelson), **s, **targets,
        )
...
def summarize(frame: pd.DataFrame, groups: list[str]) -> pd.DataFrame:
    ...
    for key, block in frame.groupby(groups, dropna=False, sort=True):
        ...
        for policy in POLICIES:
            ...
            records.append(dict(zip(groups, key)) | {
                "policy": policy, "jobs": n, "selected": n_selected,
                "correct_selected": good, "wrong_selected": n_selected - good,
    ...
    for i, row in enumerate(primary.itertuples()):
        ...
        for budget in BUDGETS:
            for repeat in range(REPEATS):
                ...
                records.append({
                    "row_id": row.row_id, "algorithm": row.algorithm,
                    "config_id": row.config_id, "backend_name": row.backend_name,
                    "target_group": row.target_group, "budget": budget, "repeat": repeat,
                    "candidate_correct": candidate in expected,
                    "holdout_unique_leader": hold_count > hold_other,
                    "holdout_candidate_mass": hold_count / remaining.sum(), **s,
...
def summarize_splits(splits: pd.DataFrame) -> pd.DataFrame:
    ...
    for (group, budget), block in splits.groupby(["target_group", "budget"]):
        for policy in POLICIES:
            ...
            records.append({
                "target_group": group, "budget": budget, "policy": policy,
                "jobs": block.row_id.nunique(), "repeats": REPEATS,
...
def controls() -> pd.DataFrame:
    ...
    for width in (2, 8, 16, 28):
        for shots in (64, 256, 1024):
            for repeat in range(1000):
                ...
                s = screen(counts)
                records.append({
                    "scenario": "uniform", "width": width, "shots": shots,
                    "repeat": repeat, **s,
                    **{f"pass_{name}": flag for name, flag in policies(s).items()},
                })
    ...
    result.groupby(["scenario", "width", "shots"]).agg(
        **{name: (f"pass_{name}", "mean") for name in POLICIES},
        mean_contrast=("contrast", "mean"), mean_gap=("gap", "mean"),
...
def checks(frame: pd.DataFrame) -> dict:
    ...
    for threshold in (0.2, 0.5, 0.8):
        ...
        assert np.array_equal(c >= threshold - 1e-12,
                              margin >= 2 * threshold / (1 + threshold) - 1e-12)
...
def figures(frame: pd.DataFrame, splits: pd.DataFrame) -> None:
    ...
    for group, block in primary.groupby("target_group"):
        axes[0].scatter(block.target_dsr, block.contrast, s=15, alpha=0.45,
                        label=f"{group} target ({len(block)})", c=colors[group])
    axes[0].plot([0, 1], [0, 1], "k--", linewidth=0.7)
    axes[0].set(xlabel="Current target-aware DSR", ylabel="Label-free top-two contrast",
                title="Target-aware DSR versus observed contrast")
    axes[0].legend(fontsize=8)
    for correct, color, label in ((True, "#1b9e77", "Correct leader"),
    ...
    axes[1].set(xlabel="Label-free top-two contrast", ylabel="Observed leading probability",
    ...
    policy_labels = {"contrast_0.2": "Contrast ≥ 0.2", "contrast_0.5": "Contrast ≥ 0.5",
    ...
    for ax, group in zip(axes, ("singleton", "multiple")):
        ...
        ax.set(xlabel="Discovery shots (remainder held out)", ylabel="Fraction of jobs selected",
               title=f"{group.capitalize()} target tasks", ylim=(0, 1))
...
def main() -> None:
    ...
    diagnostics = {
        ...
        "mean_zero_sum_positive": int(((multi.target_dsr == 0)
                                       & (multi.target_sum_signed > 0)).sum()),
        "mean_positive_min_nonpositive": int(((multi.target_dsr > 0)
                                              & (multi.target_min_signed <= 0)).sum()),
        "mean_positive_missing_target": int(((multi.target_dsr > 0)
                                             & (multi.target_coverage < 1)).sum()),
        "dsr_at_least_0.8_top2_below_0.2": int(((multi.target_dsr >= 0.8)
                                               & (multi.contrast < 0.2)).sum()),
    }
    (OUT / "target_aggregation_diagnostics.json").write_text(json.dumps(diagnostics, indent=2)
                                                          + "\n")
    ...
    numeric = ["candidate_correct", "holdout_unique_leader", "holdout_candidate_mass",
               "contrast", "gap", "rank_p"] + [f"pass_{p}" for p in POLICIES]
    split_records.groupby(["row_id", "algorithm", "config_id", "backend_name",
                           "target_group", "budget"])[numeric].mean().reset_index().to_csv(
        OUT / "split_per_job.csv", index=False)
    ...
    manifest = {
        ...
        "seed": SEED, "split_repeats": REPEATS, "split_budgets": BUDGETS,
        ...
        "rows": len(frame), "cohorts": frame.cohort.value_counts().to_dict(),
        "register_alignment_rows": int(frame.register_aligned.sum()),
        "policies": POLICIES, "validation": validation,
