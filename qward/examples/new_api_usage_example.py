...
def main():
    ...
    circuit_structure_fig = visualizer.generate_plot(
        Metrics.QISKIT, 
        Plots.QISKIT.CIRCUIT_STRUCTURE,
        save=True,
        show=False
    )
    print(f"   ✅ Generated circuit structure plot")
    ...
    analysis_results = visualizer.generate_plots(
        analysis_selections, 
        save=True, 
        show=False
    )
    
    for metric, plots in analysis_results.items():
    ...
    all_complexity_plots = visualizer.generate_plots(
        {Metrics.COMPLEXITY: None},  # None = all plots
        save=True,
        show=False
    )
    ...
    variant_visualizer = Visualizer(
        scanner=variant_scanner, 
        output_dir="qward/examples/img/variant"
    )
