"""
Clean teleportation example using the refactored vTP.py and QuantumCircuitExecutor.
...
from qward.algorithms.vTP import (
...
def basic_teleportation_example():
    ...
    generator = TeleportationCircuitGenerator(
        payload_size=2,
        gates=['x', 'h'],  # Apply X and H gates
        protocol_type='variation'
    )
    ...
    results = executor.simulate(generator.circuit, success_criteria=teleportation_success, show_results=True)
...
def protocol_comparison_example():
    ...
    for name, protocol in protocols.items():
        ...
        generator = TeleportationCircuitGenerator(
            protocol=protocol,
            payload_size=1,
            gates=['x']  # Simple X gate test
        )
        
        # Simulate
        result = executor.simulate(generator.circuit, success_criteria=teleportation_success, show_results=True)
        results[name] = result
...
def custom_gates_example():
    ...
    generator = TeleportationCircuitGenerator(
        payload_size=3,
        gates=5,  # Generate 5 random gates
        protocol_type='variation'
    )
...
def main():
    ...
    print("The vTP.py file is now clean and modular!")
