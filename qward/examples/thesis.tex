...
Recent research has addressed related challenges from a software engineering perspective. Gheorghe-Pop et al.~\cite{gheorghePopQuantumDevops_2020} proposed Quantum DevOps, a framework that integrates reliability checks for NISQ backends into the development and operation of quantum applications by testing basic gate computations before deploying larger workloads. Pérez-Castillo et al.~\cite{perezCastilloGuidelinesIncremental_2024} adapted the Incremental Commitment Spiral Model (ICSM) for quantum-classical systems, emphasizing lifecycle management, risk-based decisions, and quality assurance in hybrid software development. Khan et al.~\cite{khanAgileMeetsQuantum_2024} developed a prediction model for the success of agile quantum software projects, focusing on project-level risks and organizational factors. These contributions strengthen the software engineering treatment of quantum applications, but they operate primarily at the process, lifecycle, or project level. They do not provide a systematic method for estimating the expected execution quality of a specific quantum circuit from its structural properties.
...
At a broader level, recent literature review evidence confirms that the evaluation of quantum software remains fragmented. Desdentado et al.~\cite{desdentadoQuantumComputingSoftware_2025} report that current quantum research is centered on machine learning and optimization, while the evaluation of quantum solutions focuses on algorithm efficiency, verification, and resource optimization. However, they identify the absence of standardized evaluation methods, together with hardware constraints, noise, scalability, integration, error correction, and problem formulation, as limitations. Complementarily, Mnaathr and Hasan~\cite{mnaathrIntegratingQuantumComputing} frame quantum cloud computing as a model for remotely accessing quantum resources, while also emphasizing qubit stability, noise reduction, error correction, and resource utilization as barriers to practical quantum cloud services.

The rest of the section provides an overview of relevant works organized along two complementary lines of inquiry. The first examines ML-based approaches, detailing their inputs, targets, and architectures. The second reviews analytical and lightweight modeling alternatives. The section concludes by articulating how this thesis differentiates itself and contributes to the state of the art.
...
This economic barrier is amplified by the cloud-based delivery model that currently dominates access to QC resources \cite{mnaathrIntegratingQuantumComputing}. The QPU resources are accessed via remote cloud services due to hardware limitations and deployment constraints. Pre-runtime quality estimation addresses this challenge by enabling developers to estimate circuit performance before its execution in a quantum provider, thereby reducing unnecessary executions and focusing costly cloud resources on circuits more likely to produce meaningful results.
...
\item \textbf{Complementary Benchmarking Signal:} Paradigm-specific circuit executions provide complementary information about algorithm execution quality that is not fully captured by random-circuit benchmarks alone.
\label{h:4}
\end{enumerate}
...
\item To determine whether paradigm-specific circuit executions contribute complementary predictive information beyond what random-circuit volumetric benchmarks capture.
\label{o:4}
\end{enumerate}

The hypotheses will be validated according to the plan defined in \ref{section:work}. In summary, hypothesis~\ref{h:1} is tested by constructing the circuit footprint and measuring its predictive accuracy against post-runtime metrics. Hypothesis~\ref{h:2} is evaluated by testing whether models trained on selected circuit instances generalize to unobserved configurations within the same algorithmic paradigms. Hypothesis~\ref{h:3} is examined through cross-provider prediction experiments that compare circuit-only, calibration-only, and mixed feature configurations. Finally, Hypothesis~\ref{h:4} is evaluated by comparing random-circuit-only, paradigm-specific-only, and combined training configurations, determining whether paradigm-specific executions contribute to predictive signals not captured by random-circuit benchmarks alone.
...
This study \cite{ieeevTp2025} provided preliminary empirical evidence of a relationship between selected pre-runtime circuit characteristics and post-runtime execution outcomes. The experiment included the creation of a benchmarking framework to systematically test circuit reliability by measuring success rates across different configurations. To this end, we designed a variation of the teleportation protocol, referred to as vTP, that enables testing of quantum teleportation on a single QPU.
...
Together, the three studies above motivate the predictive framework developed in this thesis. The systematic mapping study identified the research opportunity, the qWard library provided the tooling for metric extraction and classification, and the vTP experiment provided preliminary evidence that selected circuit-structure metrics correlate with execution reliability.
...
Regarding the random-circuit comparison (\ref{h:4}), the underlying rationale is that each algorithmic paradigm produces a distinct structural scaling shape---in terms of depth profile, entanglement pattern, and gate composition---that is not uniformly sampled by the benchmarking using random circuit. Consequently, paradigm-specific benchmark executions may reveal hardware-dependent performance that are not captured by randomly generated workloads, thereby providing empirically guidance regarding which backend is most appropriate for their target algorithm.
...
\begin{longtable}{@{}p{\textwidth}@{}}
    ...
    \begin{enumerate}[label=T8.\arabic*, ref=T8.\arabic*, font=\bfseries, leftmargin=3.5em, labelindent=0pt, itemindent=0pt, labelsep=0.5em, noitemsep, topsep=0pt]
        ...
        \item \label{p8:t4} Compare random-circuit-only, paradigm-specific-only, and combined training configurations.
    ...
    \begin{itemize}[leftmargin=1.5em, labelindent=0pt, itemindent=0pt, noitemsep, topsep=0pt]
        ...
        \item Complementary predictive value of paradigm-specific and random-circuit executions assessed (\ref{h:4}).
