...
The entanglement-based paradigm includes the variation of the Teleportation Protocol (vTP) and the GHZ state preparation, which isolates the entanglement operation (the same operation used to scale up the payload in vTP), providing a simpler baseline to study how entanglement degrades as quantum circuits scale.
...
\item \textbf{Entanglement-based (vTP, GHZ).} The vTP protocol scales the payload size from 1 to $n$ qubits, while GHZ scales the number of entangled qubits. These algorithms provide a baseline for studying how entanglement degradation affects post-runtime metrics as circuit size grows.
\end{enumerate}

For all paradigms, the dataset records the four feature groups described in Table~\ref{tab:dataset-structure}. All circuits will be executed using optimization level 3, and the pre-runtime metrics will be evaluated following the transpilation stage. The validation strategy employs a temporal split in which models are trained on earlier executions and validated against subsequent executions collected on different calibration cycles, ensuring that the model does not overfit to a specific hardware snapshot.
...
\begin{longtable}{@{}p{\textwidth}@{}}
    ...
    \textbf{Description:} Close the GBMF loop by empirically validating the prediction system. Following the framework's validation criteria, the models' predictions are compared against ground-truth data obtained from newly generated QPU executions to evaluate generalization across the selected algorithms and paradigms, and to assess the role of hardware context in cross-provider prediction. This phase determines whether the ``Goal'' established in \hyperref[p:1]{Phase~1} has been met. \newline
    \textbf{Tasks:}
