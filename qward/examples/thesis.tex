...
\item \textbf{Scalability Generalization:} Models trained on instances of the selected algorithms (QFT, GO, BV, vTP, GHZ) generalize to unobserved circuit configurations, given that the new jobs use circuits within the same QC paradigm.
\label{h:2}
...
Because the available dataset is limited, the models will be trained using all collected execution records. Validation will be performed using newly generated execution data that is not used during model construction. The predicted quality metrics will then be compared against the observed outcomes of those new executions.
...
The central doctoral contribution of this thesis is the \textbf{Quality-Aware Quantum Software (QAQS) framework}, a pre-runtime quality model for quantum software that links a circuit's structural properties to its expected execution outcomes. The contributions listed below are organized into the foundational studies that enabled QAQS and the components that constitute it.

\subsection{Foundational Studies}

The following completed works established the conceptual, empirical, and tooling foundations required to construct and validate the QAQS framework.
...
\subsection{Doctoral Contribution: The QAQS Framework}

The central doctoral contribution is the QAQS framework itself, which integrates the foundational studies above into a single validated quality model. QAQS comprises the following components:
...
\item \textbf{Predictive Pipeline:} A software tool that integrates metric extraction via qWard, footprint validation, and ML-based prediction into a unified pipeline, as described in Section~\ref{section:qaqs} and Figure~\ref{fig:framework-overview}.
\label{c:framework}

\item \textbf{Validation Dataset:} An open multi-paradigm, multi-provider dataset that extends existing datasets (\ref{c:dataset1}) to cover six algorithms in four paradigms on IBM, Rigetti, and IonQ hardware, designed to validate the predictive power of the circuit footprint.
...
This research has also produced tangible outcomes in the form of undergraduate theses supervised within the scope of the qWard project at Universidad de los Andes, demonstrating the framework's applicability in educational settings.
...
\item Rodr\'{i}guez Castillo \cite{rodriguezCastilloScalabilityQuantum_2026} analyzed the scalability behavior of quantum algorithms implemented as circuits and subroutines, providing empirical evidence on how circuit structure evolves with problem size.
...
\begin{longtable}{@{}p{\textwidth}@{}}
    ...
    \textbf{Description:} Close the GBMF loop by empirically validating the prediction system. Following the framework's validation criteria, the models' predictions are compared against ground-truth data obtained from newly generated QPU executions to evaluate generalization across the selected algorithms and paradigms, and to assess the role of hardware context in cross-provider prediction. This phase determines whether the ``Goal'' established in Phase~\ref{p:1} has been met. \newline
    \textbf{Tasks:}
    \begin{enumerate}[label=T8.\arabic*, ref=T8.\arabic*, font=\bfseries, leftmargin=*, noitemsep, topsep=0pt]
        \item \label{p8:t1} Execute the target algorithms on real hardware to collect new validation data.
        \item \label{p8:t2} Compare the predicted quality metrics against the observed execution results.
        \item \label{p8:t3} Compare validation results across pre-runtime-only, hardware-only, and mixed input configurations.
        \item \label{p8:t4} Quantify predictive power and identify framework limitations.
    \end{enumerate}
    \textbf{Acceptance Criteria:}
    \begin{itemize}[leftmargin=*, noitemsep, topsep=0pt]
        \item Validation dataset collected from newly generated executions on real hardware.
        \item Predicted quality metrics compared against observed outcomes for the selected algorithms within the target paradigms (validating \ref{h:2}).
        \item Prediction results compared across pre-runtime-only, hardware-only, and mixed configurations to assess the role of hardware context in cross-provider prediction (validating \ref{h:3}).
        \item Predictive power and framework limitations documented using $R^2$, $MAE$, and $RMSE$.
    \end{itemize} \\
    \midrule
